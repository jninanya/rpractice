...
smr <- d3 %>%
  group_by(GENO, IND) %>%
  summarise(Y_mean = mean(YIELD),
            Y_cv = sd(YIELD)/mean(YIELD)*100,
            Y_min = min(YIELD))
...
smr <- d0 %>%
  group_by(FACTOR1, FACTOR2) %>%
  summarise_at(vars(c("CC","FTY","MTY","BCR","CF")), sd)
...
smr <- d0 %>%
  group_by(FACTOR1, FACTOR2) %>%
  summarise_if(is.numeric, ~sd(.)/mean(.)*100)
...
# Comparaciones multiples TUKEY: Default stats
TK <- TukeyHSD(modelo)

# Comparaciones multiples LSD: Con agricolae
...
# Homogeneidad de varianzas: p > 0.05 (Varianzas homogeneas)
bartlett.test(YIELD ~ TRT, data = d2)
...
out1 <- HSD.test(modelo, "TRT", group = TRUE)
out2 <- LSD.test(modelo, "TRT", p.adj = "none") # 
out3 <- duncan.test(modelo, "TRT")              #


### DISENO: ARREGLO FACTORIAL  ------------------------------------------
head(d3)
str(d3)
...
modelo <- aov(YIELD ~ GENO + IND + GENO:IND + REP, data = d3)
sm <- summary(modelo)
...
# Comparaciones multiples LSD
out1 <- HSD.test(modelo, "GENO", group = TRUE, console = TRUE)
out2 <- HSD.test(modelo, "IND", group = TRUE, console = TRUE)

(duncan.test(modelo, "GENO"))
(duncan.test(modelo, "IND"))
